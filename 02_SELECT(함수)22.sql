/*
    ...
    단일행 함수와 그룹 함수는 함께 사용할 수 없다 - 결과 행수가 다르기 때문.
...
FROM DUAL;-- 가상테이블로 산술 연산이나 가상 컬럼등 값을 한번만 출력하고자 할때 사용하는 테이블 
...
FROM DUAL;--앞에서부터 첫번째에 위치하는 B의 위치값을 알려준다. 찾을 위치,순번 생략시 기본적으로 앞에서부터 첫번째 위치

SELECT INSTR('AAABAABBAA','B',1)
FROM DUAL;

SELECT INSTR('AAABAABBAA','B',-1)
FROM DUAL;-- 시작값을 -1로하면 뒤에서부터 첫번째로 오는 문자를 찾아 앞에서부터 위치를 찾는다 
...
--자바에서처럼 위치값을 반환하지만 0부터 시작이 아니라 1부터 시작인것을 알수있다.
...
FROM DUAL; --6번위치부터 끝까지 추출

SELECT SUBSTR('HELLO WORLD!!!',7,2)
FROM DUAL;--7번위치부터 2개 추출

SELECT SUBSTR('HELLO WORLD!!!',1,5)
FROM DUAL; -- 1번위치부터 5개 HELLO

SELECT SUBSTR('HELLO WORLD!!',-5)
FROM DUAL; -- 음수 입력시 뒤에서 부터 위치찾음
...
FROM EMPLOYEE;
--이메일에서 ID부분만 추출해서 조회해보기 (@앞부분이 아이디)
SELECT EMAIL
FROM EMPLOYEE;
SELECT EMP_NAME,EMAIL,SUBSTR(EMAIL,1,INSTR(EMAIL,'@')-1) "아이디"
FROM EMPLOYEE;--INSTR로 @위치 찾아서 그 위치 전까지 잘라내기
--남자 사원들만 조회해보기 (힌트 : 조건절에도 사용 가능)
SELECT *
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) IN ('1','3'); --조건절에서 성별을 확인할 수 있는 문자를 추출하여 IN 연산자 활용 
--여자 사원들만 조회해보기
SELECT *
FROM EMPLOYEE
...
--주민등록번호에서 성별 이후문자를 *처리하여 추출해보세요.
--000101-3333333 성별까지 추출한 뒤에 주민번호 자리수만큼 *처리하여 다시 채우기 
SELECT RPAD('000101-3',14,'*')
FROM DUAL;
...
SELECT LTRIM('KKKKKKGGHHHHHH','K')
FROM DUAL;

SELECT RTRIM('KKKKKKGGHHHHHH','H')
FROM DUAL;
...
FROM DUAL;--제거시키고자 하는 문자열을 통으로 지워주는것이 아니라 문자 하나하나가 다 존재하면 지워주는 원리
...
--SELECT TRIM('AAABBCCC','A') 매개변수 제시 형식이 맞지 않음
SELECT TRIM(BOTH 'A' FROM 'AAABBCCCAAA')
FROM DUAL; -- BOTH 양쪽 삭제,생략시 기본값

SELECT TRIM(LEADING 'Z' FROM 'ZZZBBZZZ')
FROM DUAL; --LEADING : 앞쪽 삭제
...
/*
    ...
    :문자열로부터 찾을문자를 찾아 바꿀문자로 변경한 문자열을 반환
    결과값은 CHARACTER 형태로 반환
*/
...
/*
    ROUND
    -ROUND(반올림하고자하는 수,반올림할 위치) : 반올림 처리 함수
    반올림할 위치 : 소수점기준으로 아래에서 N번째 수에서 반올림하겠다(생략가능 생략시 0 소수점 첫번째 자리에서 반올림)
    결과값 NUMBER형
*/
...
--각 직원별로 근무일수 구하기(오늘날짜 - 고용일) - 시분초 계산까지 되어 소수점이 나왔었다. 지워보기
SELECT *
FROM EMPLOYEE;
...
--      ,FLOOR(MONTHS_BETWEEN(SYSDATE,HIRE_DATE))||'개월' 근무개월수
      ,FLOOR(ABS(MONTHS_BETWEEN(HIRE_DATE,SYSDATE)))||'개월' 근무개월수 --절대값 함수 활용
FROM EMPLOYEE;
...
FROM DUAL;
--현재 컴퓨터 언어세팅이 KOREAN이기때문에 오류 

--언어를 잠깐 변경하여 확인해봅시다.
--DDL (데이터 정의 언어) :CREATE,ALTER,DROP
ALTER SESSION SET NLS_LANGUAGE = AMERICAN;

--언어변경이 되었기때문에 영문으로 가능 /한글 불가능
...
--4.
--LAST_DAY(DATE) : 해당 특정 날짜 달의 마지막 날짜를 구하여 반환(결과값 DATE형)
SELECT LAST_DAY(SYSDATE)
FROM DUAL;

--이름,입사일,입사한 달의 마지막 날을 조회하기
...
FROM DUAL;-- 1234 => '01234' 빈칸을 0으로 채운다 
--(고정적인 값으로 변환된 숫자의 길이를 맞추고 싶으면 원하는 길이만큼 0으로 채움)
...
--RR : 50년 기준으로 작으면 20이붙고 크면 19가 붙는다 - (20)23 / (19)53
...
--2010년도 이후에 입사한 사원들의 사원명과 입사일을 조회해보자.
SELECT EMP_NAME,HIRE_DATE,TO_CHAR(HIRE_DATE,'YYYY"년" MM"월" DD"일" (DY)')
FROM EMPLOYEE
...
SELECT TO_DATE(000101)--101 로 인식
FROM DUAL;--0으로 시작하는 숫자는 숫자로 인식이 되기때문에 0으로 시작하는 년도는 문자열로 작성하여야한다.
...
FROM DUAL; --19950630  --두자리 년도에 RR포맷을 적용시키면 50이상이면 이전세기,50 미만이면 현재 세기로 나온다.

SELECT TO_DATE('950630','YYMMDD')
FROM DUAL; --20950630 YY는 무조건 20이(현재 세기) 붙기때문에 2095년도가 된다.

SELECT TO_DATE('000505','RRMMDD')
FROM DUAL;

/*
    CHARACTER -> NUMBER
    -TO_NUMBER(CHARACTER,포맷) : 문자형데이터를 숫자형으로 변환(결과값은 NUMBER형)
*/

--자동형변환의 예시 (문자열 ->숫자)
SELECT '123'+123
FROM DUAL;--246 : 자동형변환 후 산술 연산

SELECT '10,000' +'123,000'
FROM DUAL; -- 숫자에 문자 , 가 포함되어있기 때문에 자동형변환이 되지 않는다.
...
--해당 컬럼값이 존재할경우(NULL이 아닐경우) 기존의 컬럼값을 반환
--해당 컬럼값이 존재하지 않는 경우 (NULL인 경우) 내가 제시한 특정반환값을 반환

--사원명,보너스 조회를 했을때 보너스가 없는 사원은 NULL이 나왔다
--이 상황에 NULL대신 0의 값으로 조회해보자.
SELECT EMP_NAME,BONUS,NVL(BONUS,0)
FROM EMPLOYEE;

--보너스 포함 연봉 조회
--SELECT EMP_NAME,(SALARY+(SALARY*BONUS))*12 AS "보너스 포함 연봉" 
...
--NVL2(컬럼명,결과값1,결과값2)
--해당 컬럼값이 존재할 경우(NULL이 아닐경우) 결과값 1 반환
--해당 컬럼값이 존재하지 않을 경우(NULL인 경우) 결과값 2 반환

--보너스가 있는 사원은 보너스 있음, 보너스 없는 사람은 보너스 없음
SELECT EMP_NAME,BONUS,NVL2(BONUS,'보너스 있음','보너스 없음') 보너스유무
FROM EMPLOYEE;

--사원명,부서코드(부서코드가 있는 경우 '부서배치완료' 없는경우 '없음') 조회해보세요
SELECT EMP_NAME,DEPT_CODE,NVL2(DEPT_CODE,'부서배치완료','없음') AS 부서여부
FROM EMPLOYEE;
...
--사번,사원명,주민번호,번호에서 성별 자리 추출후 1이면 남자 2면 여자 조회
--SELECT EMP_ID,EMP_NAME,EMP_NO,SUBSTR(EMP_NO,8,1) 성별
--DECODE의 비교대상 위치에 성별자리 추출하여넣기 
...
/*
    ...
    -CASE WHEN THEN구문의 경우 특정 조건을 마음대로 제시할 수 있다.
...
SELECT EMP_ID 사번
      ...
      ,CASE WHEN JOB_CODE = 'J7' THEN (SALARY+(SALARY*0.1))
        ...
        END "인상 후 급여"
FROM EMPLOYEE;
--사원명,급여,급여등급(SAL_LEVEL 컬럼 사용하지 않고 직접 넣기)
--급여등급 SALARY 값이 500만원 초과인 경우 '고급'
--                    500만원 이하 350초과인경우 '중급'
--                     350만원 이하인 경우 '초급'
